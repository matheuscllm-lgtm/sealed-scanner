...
- [x] (2026-06-05) **Pool_fill freight-free por default** — operador perguntou "como você sabe o frete do lote?" e estava certo: o frete do pool era uma ESTIMATIVA calibrada (5% + R$17/loja, ancorada nos R$250/350 @ R$5k que ele deu uma vez), não frete medido — mesma categoria do líquido. Novo `freight_model="none"` no `pool_fill` (zera o frete e roteia pelo caminho per_seller → ranking por preço puro); `config.frete.modelo: none` vira o default. Aba *Pool Analysis* perde as colunas Frete/Outlay e renomeia "Preço efetivo"→"Preço médio", "Margem real"→"Margem total"; *Assumptions* mostra "freight-free". Engine ainda suporta flat/per_seller via config (tests cobrem). Demonstração PHF pack @ R$5k: none 47,5% (frete R$0) vs flat 41,0% (frete R$335 estimado). 27/27 testes (1 novo).
- [x] (2026-06-05) **ROI líquido excluído de vez (core + snapshots)** — operador: "exclua essa ideia de roi líquido, apenas com a diferença bruta de preço". `compute_margin` agora só calcula bruto (removidos `net_profit`/`net_margin`, `pct_fees`, `flat_fees`, `tax`); campos `net_*` saíram do `ScanRow`; o bloco `fees:` do `config.yaml` foi removido (só alimentava o líquido), junto das linhas de taxa/frete da aba Assumptions. `snapshot.py` perdeu a coluna "Lucro líq R$"; `snapshot_friendly.py` reescrito gross-only (Margem total + Δ R$/unid bruto; glossário sem ROI). Verificado: mock scan + os 2 snapshots geram sem erro; 26/26 testes. (O frete do `pool_fill` virou freight-free — ver item acima.)

...
- [x] (2026-06-07) **ML busca POR LOJA (`search_mode: stores`)** — em vez de varrer o ML inteiro, busca `pokemon` DENTRO de cada loja confiável (`lista.ml.com.br/loja/{slug}/{query}`, padrão extraído do JS do ML, validado: escopo 100% — `loja/asmodee/pokemon` = só Asmodee). Mais recall e precisão. Config com 5 lojas oficiais (asmodee, brink-center/Galápagos, baby-world, pokemon, copag). Scan real: 151 anúncios, 8 selados-EN in-escopo — **só Asmodee + Brink Center carregam inglês**; Loja Oficial Pokémon/COPAG/Baby World vêm PT-BR (rejeitados). Melhor margem ~+3% → **0 GREEN** (reconfirma ML ≈US). Lojas NÃO-oficiais (FLOR/BTPRIME/WBSHOP/THE_COLLECTOR) não têm `/loja/{slug}` → filtro por seller_id fica como evolução. 115 testes verdes.
